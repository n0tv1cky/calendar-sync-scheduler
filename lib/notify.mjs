...
// course name + session pill, when, and (for updates) a compact diff list.
function renderCard({ accentColor, summary, start, timeZone, body }) {
  return `
    <tr>
          ...
          <tr>
            ...
            <td style="padding:14px 16px;">
              <div style="font-size:14px; font-weight:600; color:#1a1a1a;">
                ...
                ${sessionLabel(summary) ? `<span style="font-weight:500; font-size:12px; color:#5f6368; background:#f1f3f4; border-radius:10px; padding:2px 8px; margin-left:6px;">${escapeHtml(sessionLabel(summary))}</span>` : ""}
              </div>
...
export function buildChangeSummaryEmail({ term, calendarLink, timeZone, result }) {
  ...
  const createdSection = renderSection("New sessions", "#188038", [...result.created, ...result.adopted], (e) =>
    renderCard({ accentColor: "#34a853", summary: e.summary, start: e.start, timeZone }));
  ...
  const updatedSection = renderSection("Changed sessions", "#e37400", result.updated, (e) =>
    renderCard({
      ...
      start: e.start,
      timeZone,
      body: e.changes.filter((c) => c.field !== "description").map(renderDiffLine).join("") || `<div style="font-size:12.5px; color:#5f6368; margin-top:6px;">Description updated</div>`,
    }));
